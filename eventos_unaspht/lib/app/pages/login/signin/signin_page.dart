...
import 'package:eventos_unaspht/app/pages/login/recovery_pass/recover_pass_page.dart';
import 'package:eventos_unaspht/app/pages/login/signup/signup_page.dart';
...
class _SigninPageState extends State<SigninPage>
  ...
  void didChangeDependencies() {
    ...
    animTransformEmail = Tween<double>(
      begin: 0, end: 380).animate(
        CurvedAnimation(
        parent: _animationController,
        curve: Interval(0, 0.5, curve: Curves.easeIn)));
    animTransformPassword = Tween<double>(
      begin: 0, end: -380).animate(
        CurvedAnimation(
        parent: _animationController,
        curve: Interval(0, 0.5, curve: Curves.easeIn)));
    animButtonSigninCircular = Tween<double>(
      begin: 10, end: 50).animate(
        CurvedAnimation(
        parent: _animationController,
        curve: Interval(0, 0.5, curve: Curves.easeIn)));
    animButtonTransform = Tween<double>(
      begin: 0, end: -100).animate(
        CurvedAnimation(
        parent: _animationController,
        curve: Interval(0.5, 1, curve: Curves.easeOutExpo)));
    animOpacityForgetPass = Tween<double>(
      begin: 1, end: 0).animate(
        CurvedAnimation(
        parent: _animationController,
        curve: Interval(0, 0.2, curve: Curves.easeIn)));
    animButtonWidth = Tween<double>(
      begin: MediaQuery.of(context).size.width, end: 50).animate(
        CurvedAnimation(
          parent: _animationController,
          curve: Interval(0, 0.5, curve: Curves.easeIn)));
  }
